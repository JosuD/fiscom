...
int    i,j, l, n, k, z, ps, *red, *clusters, *tamanios, dondepercola, cant_clusters, *vector_l;
...
red=(int *)malloc(100 * 100 *sizeof(int));
masa_l = (double *)malloc(20*sizeof(double));
vector_l = (int *)malloc(20*sizeof(int));
	for(i=1; i<20; i++){ *(vector_l+i) = i*5; printf("vector_l = %d\n", *(vector_l+i));}
	for(i=0; i<20; i++) *(masa_l+i) = 0.0;
	printf("Hasta aca anda, pcrit = %f\n", pcrit);
	for(i=0; i<20; i++){
	printf("comienzo largo = %d\n", *(vector_l+i));
		for(j=0; j<z; j++){
			for(l = 0; l< n*n; l++) *(clusters+l) = 0;
			llenar(red, *(vector_l+i), pcrit);
//			printf("voy a imprimir la red de tamanio l = %d\n", *(vector_l+i));
//			imprimir(red, *(vector_l+i), *(vector_l+i));
//			printf("etiqueto\n");
			hoshen(red, *(vector_l+i), clusters);
//			printf("me fijo si percola\t");
			if(percola(red,*(vector_l+i))){
				dondepercola = donde_percola(red, *(vector_l+i));
				*(masa_l+i) += intensidad(clusters, dondepercola, *(vector_l+i))/(double)z;
//				printf("largo = %d, y masa = %f\n", *(vector_l+i), *(masa_l+i));
			}
//			else printf("no pecola :( \n");
//			printf("libero red, termino iteracion %d\n", j);
			//free(red);
		}
	}

			//red=(int *)malloc(10 * 10 *sizeof(int));
			
  exportar_vector(masa_l, 20, "masa_l.txt");
  time(&t_final);
