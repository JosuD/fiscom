...
double *masa_l, s_sigma, *zetas, *tamanios_s1, *tamanios_s2, *tamanios_s3, ns1_pc, ns2_pc, ns3_pc;
char   file[10];
...
for(i=0;i<z;i++){
	...
	ns2_pc = (double)*(tamanios+10)/(double)z;
	ns3_pc = (double)*(tamanios+15)/(double)z;
...
for(i=0;i<ps; i++){
	...
	for(j=0; j<z; j++){
		...
		*(tamanios_s2+i) += (double)*(tamanios+10)/(double)z/ns2_pc;
		*(tamanios_s3+i) += (double)*(tamanios+15)/(double)z/ns3_pc;
		}
	}
